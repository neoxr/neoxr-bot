...
class Scraper {
	...
	play(query, audio = true, combine = false) {
		...
		try {
			...
			if (typeof yt == 'undefined') return resolve({ creator: global.creator, status: false, msg: 'Not found!' })
	...
	urban(q) {
		...
		try {
			...
			if (content.lenght == 0 || author.lengh == 0) return resolve({ creator: global.creator, status: false })
			resolve({ creator: global.creator, status: true, data: { content: content[0].trim(), author: author[0].trim() }})
		} catch (e) {
			console.log(e)
