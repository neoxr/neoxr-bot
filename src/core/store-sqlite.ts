...
class Store {
   ...
   private insertGroupMetadataStmt: any = null
   private getGroupMetadataStmt: any = null
   private deleteGroupMetadataStmt: any = null
   private getAllGroupMetadataIdsStmt: any = null
   private preloadGroupMetadataStmt: any = null
   ...
   private preloadGroupMetadata(): void {
      if (!this.db || !this.preloadGroupMetadataStmt) return
      try {
         const rows = this.preloadGroupMetadataStmt.all() as { id: string, data: string }[]
         const now = Date.now()
         for (const row of rows) {
   ...
   public loadGroupMetadata(jid: string): any | null {
      ...
      if (this.db && this.getGroupMetadataStmt) {
         try {
            const row = this.getGroupMetadataStmt.get(id) as { data: string } | undefined
            if (row) {
