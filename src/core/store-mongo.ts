...
class Store {
   ...
   private groupMetadataCollection: any = null
   ...
   private async preloadGroupMetadata(): Promise<void> {
      if (!this.groupMetadataCollection) return
      try {
   ...
   public addGroupMetadata(groupId: string, metadata: any): void {
      ...
      if (this.groupMetadataCollection) {
         this.groupMetadataCollection.updateOne(
            { id },
            { $set: { data: cleaned, updated_at: Date.now() } },
            { upsert: true }
         ).then(() => {
