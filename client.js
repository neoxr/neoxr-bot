...
const pino = require('pino'), path = require('path')
...
const start = async () => { 
	...
	client.ev.on('connection.update', async (update) => {
		...
		if (connection === 'connecting') spinnies.add('start', { text: 'Connecting . . .' })
		if (connection === 'open') spinnies.succeed('start', { text: `Connected, you login as ${client.user.name}` })
		if (connection === 'close') lastDisconnect.error.output.statusCode !== DisconnectReason.loggedOut ? start() : spinnies.fail('start', { text: `Can't connect to Web Socket` })
		database.default({
			users: {},
			groups: {},
			chats: {},
