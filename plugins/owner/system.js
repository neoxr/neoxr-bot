exports.run = {
   usage: ['autodownload', 'chatbot', 'debug', 'multiprefix', 'online', 'self'],
   async: async (m, {
      client,
      args,
      isPrefix,
      ...
      if (system[type] == status) return client.reply(m.chat, Func.texted('bold', `🚩 ${Func.ucword(command)} hae been ${option == 'on' ? 'activated' : 'inactivated'} previously.`), m)
      system[type] = status
