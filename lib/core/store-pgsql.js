...
class Store {
    ...
    async preloadGroupMetadata() {
        if (!this.pool)
            return;
        try {
            const { rows } = await this.pool.query('SELECT id, data FROM group_metadata ORDER BY updated_at DESC LIMIT 500');
            const now = Date.now();
            for (const row of rows) {
    ...
    createChatsProxy() {
        ...
        return new Proxy(Object.create(null), {
            ...
            set: (target, prop, value) => {
                ...
                if (self.pool) {
                    self.pool.query('INSERT INTO chats (id, data, updated_at) VALUES ($1, $2, $3) ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data, updated_at = EXCLUDED.updated_at', [prop, stringify(cleanedValue), Date.now()]).catch((err) => {
    ...
    createContactsProxy() {
        ...
        return new Proxy(Object.create(null), {
            ...
            set: (target, prop, value) => {
                ...
                if (self.pool) {
                    self.pool.query('INSERT INTO contacts (jid, data, updated_at) VALUES ($1, $2, $3) ON CONFLICT (jid) DO UPDATE SET data = EXCLUDED.data, updated_at = EXCLUDED.updated_at', [prop, stringify(cleanedValue), Date.now()]).catch((err) => {
    ...
    async loadMessage(jidOrId, id) {
        ...
        if (this.pool) {
            try {
                if (targetJid) {
                    const { rows } = await this.pool.query('SELECT data FROM messages WHERE jid = $1 AND id = $2', [targetJid, targetId]);
                    if (rows.length > 0) {
                    ...
                    const { rows } = await this.pool.query('SELECT data FROM messages WHERE id = $1 LIMIT 1', [targetId]);
                    if (rows.length > 0) {
    ...
    addGroupMetadata(groupId, metadata) {
        ...
        if (this.pool) {
            this.pool.query('INSERT INTO group_metadata (id, data, updated_at) VALUES ($1, $2, $3) ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data, updated_at = EXCLUDED.updated_at', [id, stringify(cleaned), Date.now()]).then(() => {
    ...
    async loadGroupMetadata(jid) {
        ...
        if (this.pool) {
            try {
                const { rows } = await this.pool.query('SELECT data FROM group_metadata WHERE id = $1', [id]);
                if (rows.length > 0) {
    ...
    async deleteGroupMetadata(groupId) {
        ...
        if (this.pool) {
            try {
                await this.pool.query('DELETE FROM group_metadata WHERE id = $1', [id]);
    ...
    async addNode(arg1, arg2) {
        ...
        if (this.pool) {
            ...
            const current = previous
                .then(async () => {
                try {
                    await this.pool.query('INSERT INTO nodes (jid, id, tag, data, created_at) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (jid, id) DO UPDATE SET data = EXCLUDED.data, tag = EXCLUDED.tag, created_at = EXCLUDED.created_at', [jid, nodeId, tag, stringify(cleanedNode), Date.now()]);
                    await this.pool.query('DELETE FROM nodes WHERE jid = $1 AND id NOT IN (SELECT id FROM nodes WHERE jid = $1 ORDER BY created_at DESC LIMIT $2)', [jid, this.max]).catch(() => { });
                }
                catch (e) {
    ...
    async loadNode(jidOrId, id) {
        ...
        if (this.pool) {
            try {
                if (targetJid) {
                    const { rows } = await this.pool.query('SELECT data FROM nodes WHERE jid = $1 AND id = $2', [targetJid, targetId]);
                    if (rows.length > 0)
                        return parse(rows[0].data);
                }
                else {
                    const { rows } = await this.pool.query('SELECT data FROM nodes WHERE id = $1 LIMIT 1', [targetId]);
                    if (rows.length > 0)
                        return parse(rows[0].data);
                }
            }
    ...
    async loadNodes(jid, count) {
        ...
        if (this.pool) {
            try {
                let rows = [];
                if (targetJid) {
                    const { rows: res } = await this.pool.query('SELECT data FROM nodes WHERE jid = $1 ORDER BY created_at DESC LIMIT $2', [targetJid, targetCount]);
                    rows = res;
                }
                else {
                    const { rows: res } = await this.pool.query('SELECT data FROM nodes ORDER BY created_at DESC LIMIT $1', [targetCount]);
                    rows = res;
                }
                if (rows.length === 0)
                    return null;
    ...
    async getAllNodes(jid, offset = 0) {
        ...
        if (this.pool) {
            try {
                if (jid) {
                    const { rows } = await this.pool.query('SELECT data FROM nodes WHERE jid = $1 ORDER BY created_at DESC LIMIT $2', [jid, this.max]);
                    list = rows.map((row) => parse(row.data)).reverse();
                }
                else {
                    const { rows } = await this.pool.query('SELECT data FROM nodes ORDER BY created_at DESC LIMIT $1', [this.max]);
                    list = rows.map((row) => parse(row.data)).reverse();
                }
            }
            catch {
        ...
        sliced.count = async () => {
            if (this.pool) {
                try {
                    if (jid) {
                        const { rows } = await this.pool.query('SELECT COUNT(*) as count FROM nodes WHERE jid = $1', [jid]);
                        const total = parseInt(rows[0]?.count || '0', 10);
                        const actualTotal = total > this.max ? this.max : total;
                        return Math.max(0, actualTotal - offset);
                    }
                    else {
                        const { rows } = await this.pool.query('SELECT COUNT(*) as count FROM nodes');
                        const total = parseInt(rows[0]?.count || '0', 10);
                        const actualTotal = total > this.max ? this.max : total;
                        return Math.max(0, actualTotal - offset);
                    }
        ...
        if (jid && id) {
            ...
            if (this.pool) {
                ...
                const current = previous
                    .then(async () => {
                    try {
                        await this.pool.query('INSERT INTO messages (jid, id, data, created_at) VALUES ($1, $2, $3, $4) ON CONFLICT (jid, id) DO UPDATE SET data = EXCLUDED.data, created_at = EXCLUDED.created_at', [jid, id, stringify(cleaned), Date.now()]);
    ...
    async addStory(jid, story) {
        ...
        if (this.pool) {
            try {
                await this.pool.query('INSERT INTO stories (jid, id, data, created_at) VALUES ($1, $2, $3, $4) ON CONFLICT (jid, id) DO UPDATE SET data = EXCLUDED.data, created_at = EXCLUDED.created_at', [jid, storyId, stringify(cleanedStory), Date.now()]);
                if (Math.random() < 0.05) {
                    await this.pool.query('DELETE FROM stories WHERE jid = $1 AND id NOT IN (SELECT id FROM stories WHERE jid = $1 ORDER BY created_at DESC LIMIT $2)', [jid, this.max]).catch(() => { });
                }
            }
            catch (e) {
