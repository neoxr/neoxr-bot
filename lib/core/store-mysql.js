...
class Store {
    ...
    createChatsProxy() {
        ...
        return new Proxy(Object.create(null), {
            ...
            set: (target, prop, value) => {
                ...
                if (self.pool) {
                    self.pool.query('INSERT INTO chats (id, data, updated_at) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), updated_at = VALUES(updated_at)', [prop, stringify(cleanedValue), Date.now()]).catch((err) => {
    ...
    createContactsProxy() {
        ...
        return new Proxy(Object.create(null), {
            ...
            set: (target, prop, value) => {
                ...
                if (self.pool) {
                    self.pool.query('INSERT INTO contacts (jid, data, updated_at) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), updated_at = VALUES(updated_at)', [prop, stringify(cleanedValue), Date.now()]).catch((err) => {
    ...
    async loadMessage(jidOrId, id) {
        ...
        if (this.pool) {
            try {
                if (targetJid) {
                    const [rows] = await this.pool.query('SELECT data FROM messages WHERE jid = ? AND id = ?', [targetJid, targetId]);
                    if (rows.length > 0) {
                    ...
                    const [rows] = await this.pool.query('SELECT data FROM messages WHERE id = ? LIMIT 1', [targetId]);
                    if (rows.length > 0) {
    ...
    addGroupMetadata(groupId, metadata) {
        ...
        if (this.pool) {
            this.pool.query('INSERT INTO group_metadata (id, data, updated_at) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), updated_at = VALUES(updated_at)', [id, stringify(cleaned), Date.now()]).then(() => {
    ...
    async loadGroupMetadata(jid) {
        ...
        if (this.pool) {
            try {
                const [rows] = await this.pool.query('SELECT data FROM group_metadata WHERE id = ?', [id]);
                if (rows.length > 0) {
        ...
        if (jid && id) {
            ...
            if (this.pool) {
                ...
                const current = previous
                    .then(async () => {
                    try {
                        await this.pool.query('INSERT INTO messages (jid, id, data, created_at) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), created_at = VALUES(created_at)', [jid, id, stringify(cleaned), Date.now()]);
    ...
    async addStory(jid, story) {
        ...
        if (this.pool) {
            try {
                await this.pool.query('INSERT INTO stories (jid, id, data, created_at) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), created_at = VALUES(created_at)', [jid, storyId, stringify(cleanedStory), Date.now()]);
                if (Math.random() < 0.05) {
