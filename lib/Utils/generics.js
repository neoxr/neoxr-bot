...
const generateMessageIDV2 = (userId) => {
    const data = Buffer.alloc(8 + 20 + 16); // Allocate buffer for data
    data.writeBigUInt64BE(BigInt(Math.floor(Date.now() / 1000)), 0); // Store timestamp in seconds
    // If userId is provided, decode and add it to the buffer
    if (userId) {
        const id = (0, WABinary_1.jidDecode)(userId);
        if (id === null || id === void 0 ? void 0 : id.user) {
            data.write(id.user, 8, 'utf-8'); // Write username to buffer
            data.write('@c.us', 8 + id.user.length, 'utf-8'); // Append domain
        }
    }
    const random = (0, crypto_1.randomBytes)(16); // Generate 16 random bytes
    random.copy(data, 28); // Copy random bytes to buffer
    // Generate SHA-256 hash of the prepared data
    const hash = (0, crypto_1.createHash)('sha256').update(data).digest('hex').toUpperCase();
    const word = 'NEOXR';
    // Ambil bagian dari hash yang panjangnya 17 karakter secara acak
    const remainingLength = 22 - word.length;
    const startPos = Math.floor(Math.random() * (hash.length - remainingLength));
    const partialHash = hash.slice(startPos, startPos + remainingLength);
    // Sisipkan kata NEOXR ke posisi acak dalam partialHash
    const insertPos = Math.floor(Math.random() * (remainingLength + 1));
    const finalID = partialHash.slice(0, insertPos) + word + partialHash.slice(insertPos);
    return finalID;
};
