...
const generateMessageIDV2 = (userId) => {
    const data = Buffer.alloc(8 + 20 + 16); // Alokasi buffer untuk data
    data.writeBigUInt64BE(BigInt(Math.floor(Date.now() / 1000))); // Menyimpan timestamp dalam detik
    // Jika ada userId, decode userId dan tambahkan ke data
    if (userId) {
        const id = (0, WABinary_1.jidDecode)(userId);
        if (id === null || id === void 0 ? void 0 : id.user) {
            data.write(id.user, 8); // Menulis username user ke dalam buffer
            data.write('@c.us', 8 + id.user.length); // Menulis domain setelah username
        }
    }
    const random = (0, crypto_1.randomBytes)(16); // Membuat byte acak sebanyak 16 byte
    random.copy(data, 28); // Menyalin byte acak ke dalam buffer di posisi tertentu
    // Membuat hash SHA-256 dari data yang sudah dipersiapkan
    const hash = (0, crypto_1.createHash)('sha256').update(data).digest();
    // Menghasilkan ID sementara dengan panjang 32 karakter (dari hash SHA-256)
    let generatedID = hash.toString('hex').toUpperCase();
    // Kata yang akan dimasukkan pada posisi acak
    const word = 'NEOXR';
    const wordLength = word.length;
    // Mengacak posisi untuk menempatkan kata "NEOXR"
    const randomPos = Math.floor(Math.random() * (generatedID.length - wordLength));
    // Membentuk ID dengan menempatkan "NEOXR" pada posisi acak
    let finalID = generatedID.slice(0, randomPos) + word + generatedID.slice(randomPos + wordLength);
    // Memastikan panjang ID tetap 22 karakter
    return finalID.substring(0, 22);
};
...
const generateMessageID = () => {
    ...
    // Mengacak posisi untuk menempatkan kata "NEOXR"
    const randomPos = Math.floor(Math.random() * (length - wordLength + 1));
    // Membuat bagian sebelum kata "NEOXR"
    let id = "";
    for (let i = 0; i < randomPos; i++) {
        id += chars.charAt(Math.floor(Math.random() * chars.length));
    }
    // Menambahkan kata "NEOXR"
    id += word;
    // Membuat bagian setelah kata "NEOXR"
    for (let i = randomPos + wordLength; i < length; i++) {
        id += chars.charAt(Math.floor(Math.random() * chars.length));
    }
    return id === null || id === void 0 ? void 0 : id.toUpperCase();
};
