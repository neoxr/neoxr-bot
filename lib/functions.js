import { Utils } from '@neoxr/telegram'
...
Utils.hitstat = (botDb, command, sender) => {
   try {
      if (/bot|help|menu|stat|hitstat|hitdaily/.test(command)) return
      if (typeof botDb == 'undefined') return
      botDb.statistic = botDb.statistic ? botDb.statistic : {}
      if (!botDb.statistic[command]) {
         botDb.statistic[command] = {
            hitstat: 1,
            today: 1,
            lasthit: Date.now() * 1,
            sender: sender
         ...
         botDb.statistic[command].hitstat += 1
         botDb.statistic[command].today += 1
         botDb.statistic[command].lasthit = Date.now() * 1
         botDb.statistic[command].sender = sender
      }
   } catch { }
}