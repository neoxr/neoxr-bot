...
function analyze(url) {
	...
	try {
		...
		let yt = await yts({ videoId: ytr(url) })
		let form = new URLSearchParams()
		form.append('url', url)
		...
		resolve({ creator: global.creator, status: true, ...videoInfo, data: join })
	} catch (e) {
		console.log(e)
...
function yt(url, quality = '480') {
	...
	try {
		...
		if (!json.status) return resolve({ creator: global.creator, status: false, msg: 'Can\'t get metadata!' })
		let fq = []
		...
		if (quality == '480') {
			...
			if (!fq.includes(quality.toString())) return resolve({ creator: global.creator, status: false, msg: 'Video quality ' + quality + 'p is not available!' })
		}
		...
		resolve({ ...json, token: 'api-neoxr-' + json.token, data: [ { ...mp4, ..._mp4 }, { ...mp3, ..._mp3 } ] })
	} catch {
		resolve({ creator: global.creator, status: false, msg: 'Can\'t get metadata!' })
	}})
}
...
function yta(url) {
	...
	try {
		...
		delete mp3.fquality
		delete json.extractor
		resolve({ ...json, token: 'api-neoxr-' + json.token, data: { ...mp3, ..._mp3 } })
	} catch {
		resolve({ creator: global.creator, status: false, msg: 'Can\'t get metadata!' })
	}})
}
...
function ytv(url) {
	...
	try {
		...
		delete mp4.fquality 
		delete json.extractor
		resolve({ ...json, token: 'api-neoxr-' + json.token, data: { ...mp4, ..._mp4 } })
	} catch {
		resolve({ creator: global.creator, status: false, msg: 'Can\'t get metadata!' })
	}})
}
